...
export interface MetroMapProps {
  ...
  /** How many observation slots the season funds; the row always shows them all. */
  slots?: number;
}
  ...
  const startRow = (mode: DetailMode): ReactNode =>
    startGift || startKeyword || slots > 0 ? (
      <div className="flex flex-col gap-1 px-3 py-2 text-xs text-fg-2" data-testid="start-cell">
        ...
        {slots > 0 ? (
          /*
           * Every slot the season funds, filled or not. How many observations are still unspent is
           * part of the plan — an empty slot is a decision left open, not an absence worth hiding.
           * The empty ones are not pressable: pinning belongs to the item tab's slots.
           */
          <div className="flex flex-wrap items-center gap-x-3 gap-y-1.5" aria-label={t('routeObserved', lang)} data-testid="observed-line">
            {plan.start.observed.slice(0, slots).map((entry) => (
            ...
            {Array.from({ length: Math.max(0, slots - plan.start.observed.length) }, (_, i) => (
              <span
                key={`empty-${i}`}
