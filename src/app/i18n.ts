...
export const STRINGS = {
  ...
  skillsAbout: { ko: '이 탭은 무엇인가요', en: 'What this tab shows' },
  skillsAboutRead: {
    ko: '출격한 인격의 스킬이 가진 죄악 속성·공격 유형에 반응하는 기프트입니다. 기프트 설명문의 「분노 속성 스킬」·「참격 유형인 스킬 1」 같은 구절을 읽어 만들었습니다.',
    en: "Gifts that react to the sin affinity or attack type of a deployed identity's skills, read out of phrases like 「분노 속성 스킬」 or 「참격 유형인 스킬 1」 in the gift text.",
  },
  skillsAboutEffect: {
    ko: '「발동」은 그 스킬이라야 효과가 걸리는 것, 「강화」는 걸리기는 하되 그 스킬이면 더 세지는 것입니다 — 설명문의 표현으로 가른 것이라 정확하지 않을 수 있습니다.',
    en: '「Activates」 means the effect needs such a skill; 「Strengthens」 means it happens anyway and such a skill makes it stronger. Told apart by wording, so the label can be off.',
  },
  skillsAboutLimits: {
    ko: '소속에 따라 달라지는 부분과 E.G.O·방어 스킬은 세지 않습니다. 「편성 N번 인격 전용」은 이 앱에 편성 순서라는 개념이 없어 표시만 하고 판정하지 않습니다.',
    en: 'Faction-specific variations, E.G.O skills and defence skills are not counted. 「편성 N번 인격 전용」 is shown but never judged — this app has no formation order.',
  },
  skillsAboutScope: {
    ko: '이 목록은 덱이 무엇에 반응하는지만 말합니다 — 그 기프트가 루트에서 실제로 나오는지는 말하지 않습니다.',
    en: 'This list says only what the deck reacts to — not whether the route actually hands the gift over.',
  },
  ...
  skillsSlotOnly: { ko: '{list} 한정', en: '{list} only' },
  ...
  skillsGiftCount: { ko: '기프트 {n}', en: '{n} gifts' },
